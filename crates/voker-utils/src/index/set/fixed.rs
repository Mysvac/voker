//! Provide [`IndexSet`] based on [indexmap]'s implementation.
//!
//! Unlike [`indexmap::IndexSet`], [`IndexSet`] defaults to [`FixedHashState`]
...
type InternalSet<T, S> = indexmap::IndexSet<T, S>;

// -----------------------------------------------------------------------------
// IndexSet

/// New-type for [`indexmap::IndexSet`] with [`FixedHashState`] as the default hashing provider.
///
...
///
/// ```
/// use voker_utils::index::IndexSet;
///
...
/// ```
#[repr(transparent)]
pub struct IndexSet<T, S = FixedHashState>(InternalSet<T, S>);

// -----------------------------------------------------------------------------
// `FixedHashState` specific methods

impl<T: Eq + Hash, const N: usize> From<[T; N]> for IndexSet<T> {
...
impl<T> IndexSet<T> {
    ...
    pub const fn new() -> Self {
        Self(InternalSet::with_hasher(FixedHashState))
    }

    /// Create a empty [`IndexSet`] with specific capacity
    ///
    /// # Example
    ///
    /// ```rust
    /// # use voker_utils::index::IndexSet;
    /// #
    /// let map = IndexSet::with_capacity(5);
    /// #
    /// # let mut map = map;
    /// # map.insert("foo");
    /// # assert_eq!(map.get("foo"), Some("foo").as_ref());
...
impl<T, S> IndexSet<T, S> {
    /// Return inner [`indexmap::IndexSet`] .
    #[inline(always)]
    pub fn into_inner(self) -> InternalSet<T, S> {
        self.0
    }
}

impl<T, S> From<InternalSet<T, S>> for IndexSet<T, S> {
    #[inline(always)]
    fn from(value: InternalSet<T, S>) -> Self {
        Self(value)
    }
}

impl<T, S> From<IndexSet<T, S>> for InternalSet<T, S> {
    #[inline(always)]
    fn from(value: IndexSet<T, S>) -> Self {
        value.0
    }
}

// impl<T, S> Deref for IndexSet<T, S> {
//     type Target = InternalSet<T, S>;
//
//     #[inline(always)]
...
where
    InternalSet<T, S>: Debug,
{
    #[inline(always)]
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        <InternalSet<T, S> as Debug>::fmt(&self.0, f)
    }
}

impl<T, S> Default for IndexSet<T, S>
where
    InternalSet<T, S>: Default,
{
    #[inline(always)]
    fn default() -> Self {
...
impl<T, S, I> Index<I> for IndexSet<T, S>
where
    InternalSet<T, S>: Index<I>,
{
    type Output = <InternalSet<T, S> as Index<I>>::Output;
...
impl<T, S> Eq for IndexSet<T, S> where InternalSet<T, S>: Eq {}

impl<T, S, X> FromIterator<X> for IndexSet<T, S>
where
    InternalSet<T, S>: FromIterator<X>,
{
    #[inline(always)]
...
where
    InternalSet<T, S>: IntoIterator,
{
    type Item = <InternalSet<T, S> as IntoIterator>::Item;

    type IntoIter = <InternalSet<T, S> as IntoIterator>::IntoIter;
...
where
    &'a InternalSet<T, S>: IntoIterator,
{
    type Item = <&'a InternalSet<T, S> as IntoIterator>::Item;

    type IntoIter = <&'a InternalSet<T, S> as IntoIterator>::IntoIter;
...
impl<'a, T, S> IntoIterator for &'a mut IndexSet<T, S>
where
    &'a mut InternalSet<T, S>: IntoIterator,
{
    type Item = <&'a mut InternalSet<T, S> as IntoIterator>::Item;

    type IntoIter = <&'a mut InternalSet<T, S> as IntoIterator>::IntoIter;
...
impl<T, S, X> Extend<X> for IndexSet<T, S>
where
    InternalSet<T, S>: Extend<X>,
{
    #[inline(always)]
...
impl<T, S> serde_core::Serialize for IndexSet<T, S>
where
    InternalSet<T, S>: serde_core::Serialize,
{
    #[inline(always)]
...
impl<'de, T, S> serde_core::Deserialize<'de> for IndexSet<T, S>
where
    InternalSet<T, S>: serde_core::Deserialize<'de>,
{
    #[inline(always)]
...
impl<T, S> IndexSet<T, S> {
    ...
    pub fn with_capacity_and_hasher(n: usize, hash_builder: S) -> Self {
        Self(InternalSet::with_capacity_and_hasher(n, hash_builder))
    }
    ...
    pub const fn with_hasher(hash_builder: S) -> Self {
        Self(InternalSet::with_hasher(hash_builder))
    }
