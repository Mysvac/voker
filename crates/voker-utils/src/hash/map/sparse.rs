//! Provide [`SparseHashMap`] based on [hashbrown]'s implementation.
...
type InternalMap<K, V> = hb::HashMap<K, V, SparseHashState>;

/// New-type for [`HashMap`](hb::HashMap) with [`SparseHashState`] as
...
///
/// let mut scores = SparseHashMap::new();
///
/// scores.insert("a", 25);
/// scores.insert("b", 24);
/// scores.insert("c", 12);
...
/// [`HashMap`]: hb::HashMap
#[repr(transparent)]
pub struct SparseHashMap<K, V>(InternalMap<K, V>);
...
impl<K, V> SparseHashMap<K, V> {
    ...
    /// Create a empty [`SparseHashMap`] with specific capacity
    ///
    /// # Example
    ///
    /// ```rust
    /// # use voker_utils::hash::SparseHashMap;
    /// #
    /// let map = SparseHashMap::with_capacity(5);
    /// # // docs test
    /// # let mut map = map;
    /// # map.insert(0usize, "foo");
    /// # assert_eq!(map.get(&0), Some("foo").as_ref());
...
impl<K, V> Clone for SparseHashMap<K, V>
where
...
impl<K, V> Debug for SparseHashMap<K, V>
where
...
impl<K, V> PartialEq for SparseHashMap<K, V>
where
...
impl<K, V> Eq for SparseHashMap<K, V> where InternalMap<K, V>: Eq {}

impl<K, V, T> FromIterator<T> for SparseHashMap<K, V>
where
...
impl<K, V, T> Index<T> for SparseHashMap<K, V>
where
...
impl<K, V> IntoIterator for SparseHashMap<K, V>
where
...
impl<'a, K, V> IntoIterator for &'a SparseHashMap<K, V>
where
...
impl<'a, K, V> IntoIterator for &'a mut SparseHashMap<K, V>
where
...
impl<K, V, T> Extend<T> for SparseHashMap<K, V>
where
...
impl<'de, K, V> serde_core::Deserialize<'de> for SparseHashMap<K, V>
where
    ...
    K: Eq + Hash,
{
    ...
    /// to be inserted in the SparseHashMap.
    ///
    /// # Example
    ///
    /// ```rust
    /// # use voker_utils::hash::SparseHashMap;
    /// let mut map = SparseHashMap::with_capacity(5);
    ///
    /// # let mut map: SparseHashMap<(), ()> = map;
    /// #
    /// assert!(map.capacity() >= 5);
    ///
    /// map.reserve(10);
    ...
    /// to be inserted in the given `SparseHashMap<K, V>`.
    ///
    /// # Example
    ///
    /// ```rust
    ...
    /// ```
    #[inline(always)]
    pub fn entry(&mut self, key: K) -> hb::Entry<'_, K, V, SparseHashState> {
        self.0.entry(key)
    }
    ...
    /// # use voker_utils::hash::SparseHashMap;
    /// let mut map = SparseHashMap::new();
    /// # let mut map: SparseHashMap<&'static str, usize> = map;
    ///
    ...
    pub fn entry_ref<'a, 'b, Q>(
        &'a mut self,
        key: &'b Q,
    ) -> EntryRef<'a, 'b, K, Q, V, SparseHashState>
    where
    ...
    pub fn try_insert(
        ...
        key: K,
        value: V,
    ) -> Result<&mut V, OccupiedError<'_, K, V, SparseHashState>> {
