...
pub(super) struct MessageSequence<M: Message> {
    pub messages: Vec<(MessageId<M>, M)>,
    pub start_id: usize,
}

/// Resource storage for message values of type `M`.
///
/// `Messages<M>` uses two sequences:
...
impl<M: Message + Debug> Debug for Messages<M> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
...
impl<M: Message> Extend<M> for Messages<M> {
        ...
        I: IntoIterator<Item = M>,
    {
        ...
        let messages = iter.into_iter().map(|message| {
            let id = MessageId::new(msg_count);
...
impl<M: Message> Messages<M> {
    ...
    pub fn get(&self, id: usize) -> Option<(MessageId<M>, &M)> {
    ...
    ///
    /// Returns `None` if the index is already expired or not yet written.
    pub fn get_mut(&mut self, id: usize) -> Option<(MessageId<M>, &mut M)> {
        if id < self.messages_a.start_id {
            return None;
        }
    ...
    ///
    /// Returns the generated [`MessageId`] for later correlation.
    pub fn write(&mut self, message: M) -> MessageId<M> {
        let id = MessageId::new(self.counter);
