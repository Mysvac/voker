...
impl World {
    ...
    /// struct Bar(u64);
    ///
    /// let bar = world.get_resource_or_init::<Bar>();
    /// assert_eq!(bar.0, 0);
    /// ```
    pub fn get_resource_or_init<T: Resource + Send + Default>(&mut self) -> ResMut<'_, T> {
        #[cold]
        #[inline(never)]
        fn get_or_init_cold<T: Resource + Send + Default>(this: &mut World) -> ResMut<'_, T> {
            let id = this.register_resource::<T>();
            this.prepare_resource(id);
            let data = unsafe { this.storages.res.get_unchecked_mut(id) };
            let last_run = this.last_run;
            let this_run = Tick::new(*this.this_run.get_mut());
            unsafe {
                data.insert(T::default(), this_run);
            }
            unsafe {
        ...
        if let Some(id) = world_mut.resources.get_id(TypeId::of::<T>())
            && let Some(data) = world_mut.storages.res.get_mut(id)
    ...
    /// struct Bar(u64);
    ///
    /// let bar = world.get_non_send_or_init::<Bar>();
    /// assert_eq!(bar.0, 0);
    /// ```
    pub fn get_non_send_or_init<T: Resource + Default>(&mut self) -> NonSendMut<'_, T> {
        #[cold]
        #[inline(never)]
        fn get_or_init_cold<T: Resource + Default>(this: &mut World) -> NonSendMut<'_, T> {
            let id = this.register_resource::<T>();
            this.prepare_resource(id);
            let data = unsafe { this.storages.res.get_unchecked_mut(id) };
            let last_run = this.last_run;
    ...
    pub fn resource_scope<T: Resource + Send, R>(
            ...
            && let Some((ptr, mut added, mut changed)) = unsafe { data.leak() }
        {
            ...
            unsafe {
                ...
                if let Some(data) = self.storages.res.get_mut(id) {
