...
/// Fallback stub for API compatibility with multithreaded mode.
///
/// In no-std single-thread mode, `ScopeExecutor` is intentionally unavailable.
...
impl<'task> ScopeExecutor<'task> {
    ...
    /// # Panics
    ///
    /// Always panics in fallback mode.
    pub fn spawn<T: Send + 'task>(
        &self,
        _future: impl Future<Output = T> + Send + 'task,
    ) -> Task<T> {
        panic!("`ScopeExecutor` is unavailable in no_std fallback mode.")
    }
...
///
/// Cannot be used in `no_std` fallback mode.
#[derive(Debug)]
