...
/// interface. However, it uses `TaskPool` to compute batches in parallel.
///
...
///
/// # Semantic differences from `Iterator`
///
/// Some terminal methods intentionally differ from sequential `Iterator`
/// semantics:
/// - [`ParallelIterator::all`] and [`ParallelIterator::any`] are not short-circuiting.
/// - [`ParallelIterator::position`] consumes all batches.
    ...
    Self: Sized + Send,
{
    ...
    fn nth(mut self, _pool: &TaskPool, n: usize) -> Option<BatchIter::Item> {
        ...
        while let Some(batch) = self.next_batch() {
            let (min_hint, max_hint) = batch.size_hint();
            // if max_hint == Some(min_hint), we assume it's accuracy.
            if max_hint == Some(min_hint) && min_hint + i <= n {
                // if min_hint + i == n, the target is the head of next batch.
                i += min_hint;
                continue;
            }
        ...
        BatchIter::Item: Send + 'static,
    {
        ...
        let mut batches = pool.scope(|s| {
            while let Some(batch) = self.next_batch() {
                let index = batch_index;
                batch_index += 1;
                s.spawn(async move { (index, batch.collect::<Vec<_>>()) });
            }
        });
    ...
    where
        ...
        BatchIter::Item: Send + 'static,
    {
        let (mut left, mut right) = <(C, C)>::default();
