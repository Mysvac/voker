...
/// Chains two [`ParallelIterator`]s `T` and `U`.
///
/// First returning batches from `T`, and then from `U`.
#[derive(Debug)]
pub struct Chain<T, U> {
    pub(super) left: T,
    pub(super) right: U,
    pub(super) left_in_progress: bool,
}
