...
impl<P: VectorSpace<Scalar = f32>> CubicNurbs<P> {
    ...
    pub fn uniform_knots(control_points: usize) -> Option<Vec<f32>> {
        ...
        Some((0..Self::knots_len(control_points)).map(|v| v as f32).collect())
    }
...
mod tests {
    ...
    fn cardinal_control_pts() {
        ...
        assert!(curve.velocity(0.).abs_diff_eq((p1 - p0) * tension * 2., FLOAT_EQ));
        assert!(curve.velocity(1.).abs_diff_eq((p2 - p0) * tension, FLOAT_EQ));
        assert!(curve.velocity(2.).abs_diff_eq((p3 - p1) * tension, FLOAT_EQ));
        assert!(curve.velocity(3.).abs_diff_eq((p3 - p2) * tension * 2., FLOAT_EQ));
    }
