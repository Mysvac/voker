...
/// # Example
/// ```rust
/// # use voker_math::curve::*;
/// # use voker_math::curve::cores::*;
...
impl<T> UnevenCore<T> {
    ...
    pub fn new(timed_samples: impl IntoIterator<Item = (f32, T)>) -> Result<Self, UnevenCoreError> {
        ...
        let mut timed_samples =
            timed_samples.into_iter().filter(|(t, _)| t.is_finite()).collect_vec();
        timed_samples
    ...
    pub fn map_sample_times(mut self, f: impl Fn(f32) -> f32) -> UnevenCore<T> {
        let mut timed_samples = self.times.into_iter().map(f).zip(self.samples).collect_vec();
